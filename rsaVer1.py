    ...
    if st.session_state.done_recv:
        ...
        if st.button("復号", key='dec_btn'):
            try:
                ...
                size = (nv_int.bit_length() + 7) // 8
                decoded = []
                for i in range(0, len(cb), size):
                    chunk = cb[i:i+size]
                    num   = int.from_bytes(chunk, 'big')
                    m     = pow(num, dv_int, nv_int)
                    decoded.append(chr(m + 65))
                msg = ''.join(decoded)
                st.success(f"復号結果: {msg}")
            except ValueError:
                st.error("数値への変換に失敗しました。鍵が正しいか確認してください。")
            except base64.binascii.Error:
                st.error("Base64 形式の暗号文が正しくありません。")
...
# --- 送信者モード ---
    ...
    if st.button("暗号化", key='enc_btn'):
        try:
            ...
            ev_int = int(ev)
            size = (nv_int.bit_length() + 7) // 8
            cb = b''.join(pow(ord(c) - 65, ev_int, nv_int).to_bytes(size, 'big') for c in plain)
            b64 = base64.b64encode(cb).decode()
            st.subheader("暗号文 (Base64)")
            st.code(b64)
            st.session_state.cipher_str = b64
        except ValueError:
            st.error("鍵に数値以外が含まれています。正しい鍵を入力してください。")
...
# --- 一人で実験モード ---
    ...
    phi1 = (p1 - 1) * (q1 - 1)
    common_es = [3, 17, 65537]
    e1_candidates = [e for e in common_es if gcd(e, phi1) == 1]
    e1_list = e1_candidates + [i for i in range(5001, 6000) if gcd(i, phi1) == 1 and i not in (p1, q1)]
