...
# 3. 復号（一文字ずつ）
st.header("3. 復号")
n_dec = st.session_state.n
d_dec = st.session_state.d
cipher_input = st.text_input(
    "暗号文（数値リストをカンマ区切りで）", 
    placeholder="例) 12345,67890,23456"
)

if st.button("復号", key="dec"):
    if n_dec == 0:
        st.error("まず鍵を作成＆暗号化してください。")
    else:
        try:
            nums = [int(x.strip()) for x in cipher_input.split(',')]
            plain_chars = []
            for c_i in nums:
                m_i = pow(c_i, d_dec, n_dec)
